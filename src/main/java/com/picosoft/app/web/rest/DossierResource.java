...
/**
 * REST controller for managing {@link com.picosoft.app.domain.Dossier}.
 */
@RestController
@RequestMapping("/api")
public class DossierResource {
    ...
    @PutMapping("/update-dossAccepter/{annId}")
    @ResponseBody
    public void updateDemandeAccepted(@PathVariable("annId")Long annId) {

        dossierRepository.acceptAnnonceJPQL(annId);
       // return new ResponseEntity<String>("rent annonce updated successfully",HttpStatus.OK);

    }
    @PutMapping("/update-dossDenied/{annId}")
    @ResponseBody
    public void updateDemandeDenied(@PathVariable("annId")Long annId) {

        dossierRepository.DeniedAnnonceJPQL(annId);
       // return new ResponseEntity<String>("rent annonce updated successfully",HttpStatus.OK);

    }
    @GetMapping(value = "/searchdossier/{keyword}")
    public List<Dossier> dynamicSearch(@PathVariable String keyword){
       return dossierRepository.search(keyword);
    }
    ...
    public ResponseEntity<List<Dossier>> getAllDossiers(Pageable pageable) {
        ...
        Page<Dossier> page = dossierService.findAll(pageable);
